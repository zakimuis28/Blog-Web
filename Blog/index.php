...
<body>
    ...
    <!-- Header -->
    <header class="header text-center" data-aos="fade-down">
        <h1>Berita Terkini</h1>
        <p class="subtitle">Informasi &amp; Inspirasi Gen Z</p>
    </header>
    ...
    <main class="main-content">
        <div class="row g-4">
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<article class="post card h-100 shadow-lg border-0" data-aos="fade-up">';
                if (!empty($row['picture'])) {
                    echo '<img src="images/' . htmlspecialchars($row['picture']) . '" alt="' . htmlspecialchars($row['title']) . '" class="post-img card-img-top" style="height:260px;object-fit:cover;border-radius:18px 18px 0 0;">';
                }
                ...
                $catQ = mysqli_query($conn, "SELECT c.name FROM category c JOIN article_category ac ON c.id = ac.category_id WHERE ac.article_id = " . intval($row['id']) . " LIMIT 1");
                if ($cat = mysqli_fetch_assoc($catQ)) {
                    echo ' &bull; <span class="category">' . htmlspecialchars($cat['name']) . '</span>';
                }
                ...
                echo '<div>' . htmlspecialchars($ringkas) . '</div>';
                echo '<a href="detail.php?id=' . $row['id'] . '" class="read-more-btn btn btn-warning mt-3">Baca selanjutnya</a>';
                echo '</div>';
                echo '</article>';
                echo '</div>';
            }
